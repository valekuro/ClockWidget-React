 ...
 * @returns
 */
export default function Button({ borderRadius, textOnButton, iconOnButton, variant, style, backgroundColor, color, fontSize, onClick }: ButtonProps) {
  return (
    <ButtonStyle borderRadius={borderRadius} variant={variant} style={style} backgroundColor={backgroundColor} color={color} fontSize={fontSize} onClick={onClick}>
      {textOnButton ? textOnButton : iconOnButton}
    </ButtonStyle>
  );
}

export const ButtonStyle = styled.button<{ borderRadius: string; color: string | undefined; fontSize: string | undefined; variant: string; backgroundColor: string | undefined }>`
